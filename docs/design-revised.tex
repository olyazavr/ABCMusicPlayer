...
\item {\bf Listener}: While parsing the tree, we only care about these events: exit note, exit rest, exit duplet, exit triplet, exit quadruplet, exit chord, enter measure, exit measure, exit line, exit lyric, enter voice, exit header, exit tune. Several stacks are present at once, for each kind of object.
...
When exiting a line, lyrics are matched with their notes in their measure, and padded as necessary. Measure objects are created and added to the stack.

When entering and exiting a measure, repeats are searched for and the repeated notes in each measure are added to stack so they appear twice. Possible repeated notes are kept track of, and cleared after they are repeated or if an end of notes symbol is encountered.
